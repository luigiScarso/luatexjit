...
static const char _svn_version[] =
    "$Id: printing.w 4457 2012-07-13 13:16:19Z taco $"
    "$URL: http://foundry.supelec.fr/svn/luatex/trunk/source/texk/web2c/luatexdir/tex/printing.w $";
...
void print_banner(const char *v, int e, int ver)
{
    ...
    if (callback_id == 0) {
        if (ver < 0)
            fprintf(term_out, "This is LuaTeXJIT, Version %s-%d ", v, e);
        else
            fprintf(term_out, "This is LuaTeXJIT, Version %s-%d (rev %d) ", v, e,
                    ver);
        if (format_ident > 0)
...
void log_banner(const char *v, int e, int ver)
{
    ...
    if (month > 12)
        month = 0;
    if (ver < 0)
        fprintf(log_file, "This is LuaTeXJIT, Version %s-%d ", v, e);
    else
        fprintf(log_file, "This is LuaTeXJIT, Version %s-%d (rev %d) ", v, e, ver);
    slow_print(format_ident);
    print_char(' ');
    print_char(' ');
